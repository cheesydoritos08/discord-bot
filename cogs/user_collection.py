...
class User_Collection(commands.Cog):
    ...
    def __init__(self, bot : commands.Bot):
        ...
        # Create an if statement to check to see if legendary of the week loop is running and starts it if not
    ...
    async def add_character_to_inventory(self, rarity, user_id, roll_type):
        ...
        try:
            ...
            await create_error_embed(error=e, msg=f"This occured while trying to add a character to the inventory on line {line_num}")


    # Manages what happens when the user rolls on the standard banner
    async def standard_banner_roll(self, user_id):
        try:
            ...
            await create_error_embed(error=e, msg=f"This occured while running the standard banner roll function on line {line_num}")


    # Manages what happens when the user rolls on the limited time banner
    async def limited_time_banner_roll(self, user_id):
        try:
            # Creates a new profile for user if they don't already have one
            database_handler.create_new_profile(user_id=user_id)
            ...
            await create_error_embed(error=e, msg=f"This occured while running the limited banner roll function on line {line_num}")
    ...
    async def evolve_fighter_character(self, user_id, character):  
        try:   
            ...
            await create_error_embed(error=e, msg=f"This occured while evolving a fighter character on line {line_num}")

    # Evolves the support character based on rarity
    async def evolve_support_character(self, user_id, character):
        try:
            evolution_dictionary = {
                "Common": {
                    "buff": 3,
            ...
            await create_error_embed(error=e, msg=f"This occured while evolving a support character on line {line_num}")
    ...
    async def roll(self, ctx, *, banner=None):
        try:
            ...
            async def update_user_profile_stats(rarity):
                try:
                    ...
                    await create_error_embed(ctx=ctx, error=e, msg=f"This occured while updating a user profile status while rolling for a character on line {line_num}")
            ...
            await create_error_embed(ctx=ctx, error=e, msg=f"This occured while rolling for a character on line {line_num}")
    ...
    async def return_character_list(self, ctx, characters, filter): 
        try:
            ...
            await create_error_embed(ctx=ctx, error=e, msg=f"This occured while returning the list of characters on line {line_num}")
    ...
    async def user_character_collection(self, ctx, *, filter=None):
        try:
            ...
            await create_error_embed(ctx=ctx, error=e, msg=f"This occured while displaying the user character collection on line {line_num}")
    ...
    async def all_character_collection(self, ctx, *, filter=None):
        try:
            ...
            await create_error_embed(ctx=ctx, error=e, msg=f"This occured while displaying all of the characters on line {line_num}")
    ...
    async def display_inventory(self, ctx, *, searched_item : InventoryConverter = None):
        ...
        # Checks to see if the user is looking for a specific item
            ...
            await create_error_embed(ctx=ctx, error=e, msg=f"This occured while displying the inventory on line {line_num}")
    ...
    async def evolve_character(self, ctx, *, character=None):
        try:
            ...
            await create_error_embed(ctx=ctx, error=e, msg=f"This occured while evolving a character on line {line_num}")
